    ...
    <AuthenticatedLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-8">
          <ResumenCard titulo="Aprobados"    valor={resumen.aprobado} color="text-green-600"  bgColor="bg-green-50"  icon="✅" />
          <ResumenCard titulo="En Proceso"   valor={resumen.proceso}  color="text-yellow-600" bgColor="bg-yellow-50" icon="⏳" />
          <ResumenCard titulo="Morosos"      valor={resumen.moroso}   color="text-red-600"    bgColor="bg-red-50"    icon="⚠️" />
          <ResumenCard titulo="Pagados"      valor={resumen.pagado}    color="text-blue-600"   bgColor="bg-blue-50"   icon="💰" />
          <ResumenCard titulo="Monto Total"  valor={`$${totalMonto.toLocaleString()}`} color="text-purple-600" bgColor="bg-purple-50" icon="📊" isAmount />
        </div>

        {/* Botones de acción */}
        <div className="flex flex-wrap gap-3 justify-center mb-8">
          <button className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md font-medium transition-colors duration-200 flex items-center gap-2">
            <span>📄</span> Exportar PDF
          </button>
          <button className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md font-medium transition-colors duration-200 flex items-center gap-2">
            <span>📊</span> Exportar Excel
          </button>
          <button className="w-full sm:w-auto border border-blue-600 text-blue-600 hover:bg-blue-50 px-6 py-2 rounded-md font-medium transition-colors duration-200 flex items-center gap-2">
            <span>🔄</span> Actualizar Datos
          </button>
        </div>

        {/* Detalle para móvil: cards */}
        <div className="md:hidden space-y-4 mb-8">
          {filtered.map((item, idx) => (
            <div key={idx} className="bg-white rounded-lg shadow p-4">
              ...
              <p className="text-sm"><strong>Monto:</strong> ${item.monto.toLocaleString()}</p>
              <p className="text-sm"><strong>Estado:</strong> <EstadoBadge estado={item.estado} /></p>
            </div>
          ))}
        </div>

        {/* Detalle para tablet+desktop: tabla */}
          ...
          <div className="overflow-x-auto px-4 sm:px-6 lg:px-8">
            <table className="min-w-full">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ID Crédito</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Cliente</th>
                  <th className="hidden sm:table-cell px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Promotora</th>
                  ...
                  <th className="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Fecha Solicitud</th>
                  <th className="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Fecha Aprobación</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-slate-200">
                {filtered.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{item.cliente}</td>
                    <td className="hidden sm:table-cell px-6 py-4 whitespace-nowrap text-sm text-slate-700">{item.promotora}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">${item.monto.toLocaleString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap"><EstadoBadge estado={item.estado} /></td>
                    <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-slate-700">{new Date(item.fechaSolicitud).toLocaleDateString('es-ES')}</td>
                    <td className="hidden lg:table-cell px-6 py-4 whitespace-nowrap text-sm text-slate-700">{item.fechaAprobacion ? new Date(item.fechaAprobacion).toLocaleDateString('es-ES') : '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ResumenCard({ titulo, valor, color, bgColor, icon, isAmount = false }: {
  titulo: string;
  valor: number | string;
  color: string;
  bgColor: string;
...
function EstadoBadge({ estado }: { estado: string }) {
  ...
  const { bg, text, icon } = cfg[estado] || cfg['rechazado'];
  return (
